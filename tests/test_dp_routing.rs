...
mod dp_routing_tests {
    ...
    async fn test_get_dp_aware_workers_expansion() {
        ...
        for url in &expanded {
            let (base_url, dp_rank) = dp_utils::parse_worker_url(url);
            assert!(dp_rank.is_some(), "Expanded URL should have dp_rank: {}", url);
    ...
    async fn test_get_dp_aware_workers_ipv6_expansion() {
        let urls =
            vec!["https://[2a03:83e4:5006:0090:5f5a:f8c5:0400:0000]:20009".to_string()];
        let dp_size = 2;
...
mod dp_e2e_tests {
    ...
    async fn test_regular_router_dp2_transparent_proxy_reaches_backend() {
        ...
        let port: u16 = worker_url.split(':').last().unwrap().parse().unwrap();
        ...
        let app = common::test_app::create_test_app(
            Arc::clone(&router),
            Client::new(),
            &config,
        );
    ...
    async fn test_regular_router_dp2_creates_correct_worker_count() {
        ...
        let mut ranks: Vec<usize> = all_workers
            .iter()
            .filter_map(|w| w.dp_rank())
            .collect();
        ranks.sort();
    ...
    async fn test_pd_router_dp1_creates_basic_workers() {
        ...
        let all_workers = app_context.worker_registry.get_all();
        assert_eq!(all_workers.len(), 2, "PD mode dp_size=1: 1 prefill + 1 decode");
